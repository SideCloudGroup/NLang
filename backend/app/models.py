...
class CreateEntryRequest(BaseModel):
    abbr: str = Field(..., min_length=1, max_length=128, description="缩写（允许重复）")
    value: str = Field(..., min_length=1, max_length=2048, description="对应词/短语")


class UpdateEntryByAbbrValueRequest(BaseModel):
    abbr: str = Field(..., min_length=1, max_length=128, description="要匹配的缩写（原值）")
...
class UpdateEntryByAbbrValueResponse(BaseModel):
    updated: int = Field(..., ge=0, description="更新条数（按缩写+value精确匹配）")


class DeleteEntryRequest(BaseModel):
    abbr: str = Field(..., min_length=1, max_length=128, description="要删除的缩写")
    value: str = Field(..., min_length=1, max_length=2048, description="要删除的释义")


class DeleteEntryResponse(BaseModel):
    deleted: int = Field(..., ge=0, description="删除条数（按缩写+value精确匹配）")
